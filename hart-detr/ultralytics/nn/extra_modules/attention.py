...
__all__ = ['EMA', 'SimAM', 'SpatialGroupEnhance', 'BiLevelRoutingAttention', 'BiLevelRoutingAttention_nchw', 'TripletAttention', 
           ...
           'HiLo', 'LocalWindowAttention', 'ELA', 'CAA','Combination','DilateAttention','EfficientAdditiveAttnetion', 'CAFM']
...
class HiLo(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., window_size=2, alpha=0.5):
        ...
        head_dim = int(dim/num_heads)  # head_dim 表示每个注意力头处理的特征子集的维度。
        self.dim = dim
...
class Combination(nn.Module):
    def __init__(self, dim, num_heads=8, kernel_size=3, qkv_bias=False, qk_scale=None, attn_drop=0, proj_drop=0,
                 window_size=2, alpha=0.5, sr_ratio=1.0, c_ratio=1.25, dilation=[1, 2, 3, 4]):
        super().__init__()
        assert dim % num_heads == 0, f"dim {dim} should be divided by num_heads {num_heads}."
        ...
        if self.l_heads > 0:
            if self.sr_ratio > 1:
                self.l_q = nn.Linear(self.l_dim, self.c_new, bias=qkv_bias)
                self.reduction = nn.Sequential(
                    nn.Conv2d(self.l_dim, self.l_dim, kernel_size=sr_ratio, stride=sr_ratio, groups=dim),
                    nn.Conv2d(self.l_dim, self.c_new, kernel_size=1, stride=1))
                self.norm_act = nn.Sequential(
                    nn.LayerNorm(self.c_new),
                    nn.GELU())
                self.l_k = nn.Linear(self.c_new, self.c_new, bias=qkv_bias)
                self.l_v = nn.Linear(self.c_new, self.l_dim, bias=qkv_bias)
            else:
                self.l_q = nn.Linear(self.dim, self.l_dim, bias=qkv_bias)
                self.l_kv = nn.Linear(self.dim, self.l_dim * 2, bias=qkv_bias)

            # self.l_proj = nn.Conv2d(in_chans, embed_dim, kernel_size, patch_size, padding)
            self.l_proj = nn.Linear(self.l_dim, self.l_dim)

        # High frequence attention (Hi-Fi)
        if self.h_heads > 0:
            self.h_qkv = nn.Conv2d(self.dim, self.h_dim * 3, 1, bias=qkv_bias)
            self.h_proj = nn.Linear(self.h_dim, self.h_dim)

    # 2024/6/25
    def high_Dilate(self, x):
        B, C, H, W = x.shape
        # x = x.permute(0, 3, 1, 2)  # B, C, H, W
        # print("x_size=", x.size())
        # print(C)
        # qkv0 = self.h_qkv(x).reshape(B, 3, 2, self.h_dim // 2, H, W)
        # print("qkv0_size=",qkv0.size())
        # print(self.h_dim)
        qkv = self.h_qkv(x).reshape(B, 3, self.num_dilation, self.h_dim // self.num_dilation, H, W).permute(2, 1, 0, 3,
                                                                                                            4, 5)
        # num_dilation,3,B,C//num_dilation,H,W
        x = x.reshape(B, self.num_dilation, C // self.num_dilation, H, W).permute(1, 0, 3, 4, 2)
        # num_dilation, B, H, W, C//num_dilation
        # z = torch.randn(self.num_dilation,B,H,W,self.h_dim//self.num_dilation).to(device)
        z = x.shape[4]
        x = x[:,:,:,:,:(z//2)].clone()
        for i in range(self.num_dilation):
            x[i] = self.dilate_attention[i](qkv[i][0], qkv[i][1], qkv[i][2])  # B, H, W,C//num_dilation
        x = x.permute(1, 2, 3, 0, 4).reshape(B, H, W, int(C * (1 - self.alpha)))
        x = self.h_proj(x)
        x = self.proj_drop(x)
        return x
    ...
    def low_SSA(self, x):
        B, C, H, W = x.shape
        # C_l = self.l_dim
        # x = self.l_proj(x).flatten(2).transpose(1,2)
        x = x.flatten(2).transpose(1, 2)
        N = x.shape[1]
        if self.sr_ratio > 1:
            # reduction
            ...
            q = self.l_q(x).reshape(B, N, self.num_heads, int(self.c_new / self.num_heads)).permute(0, 2, 1, 3)
            k = self.l_k(_x).reshape(B, -1, self.num_heads, int(self.c_new / self.num_heads)).permute(0, 2, 1, 3)
            v = self.l_v(_x).reshape(B, -1, self.num_heads, int(C / self.num_heads)).permute(0, 2, 1, 3)
        else:
            # q0 = self.l_q1(x)
            # q0 = self.l_q(x)
            # print("q0_size=",q0.size())
            q = self.l_q(x).reshape(B, N, self.num_heads, int(C / self.num_heads * self.alpha)).permute(0, 2, 1, 3)
            kv = self.l_kv(x).reshape(B, -1, 2, self.num_heads, int(C / self.num_heads * self.alpha)).permute(2, 0, 3, 1, 4)
            k, v = kv[0], kv[1]
        ...
        # print("v_size=", attn.size())
        # x0 = (attn @ v).transpose(1, 2)
        # print(x0.size())

        x = (attn @ v).transpose(1, 2).reshape(B, H, W, int(C * self.alpha))   # B N C
        # attn:(B, num_heads, N, N')    v:(B, N', num_heads, C / num_heads)   attn @ v：(B, num_heads, N, C / num_heads)
        x = self.l_proj(x)
        x = self.proj_drop(x)
        return x
    ...
    def forward(self, x):
        ...
        if self.h_heads > 0:
            x2 = self.high_Dilate(x)
            if self.l_heads == 0:
                x = x2

        if self.l_heads != 0 & self.h_heads != 0:
...
class EfficientAdditiveAttnetion(nn.Module):
    ...
    Input: tensor in shape [B, N, D]  #N是token(Q/K)数量, D是每个token的维度
    Output: tensor in shape [B, N, D]
    """

    def __init__(self, in_dims=512, token_dim=256, num_heads=1):
        super().__init__()
        token_dim = in_dims  # token_dim: 注意力机制中token的维度，此处设置为 in_dims
...
#     def __init__(self, dim, num_heads = 8, bias=False, num_heads0 = 1):
...
#         # 一个 3D 卷积层，用于生成查询 (Q)、键 (K) 和值 (V)
#         self.qkv_dwconv = nn.Conv3d(dim * 3, dim * 3, kernel_size=(3, 3, 3), stride=1, padding=1, groups=dim * 3,
#                                     bias=bias)
#         # 一个 3D 卷积层，用于对 qkv 的结果进行深度卷积
#         self.project_out = nn.Conv3d(dim, dim, kernel_size=(1, 1, 1), bias=bias)
#         # 一个 3D 卷积层，用于将注意力机制的输出通道映射回输入的通道数
#         self.fc = nn.Conv3d(3 * self.num_heads, 9, kernel_size=(1, 1, 1), bias=True)
#         # 一个 3D 卷积层，用于处理来自自注意力机制的输出特征
#         self.dep_conv = nn.Conv3d(9 * dim // self.num_heads, dim, kernel_size=(3, 3, 3), bias=True,
#                                   groups=dim // self.num_heads, padding=1)
#         # 一个 3D 卷积层，用于进行深度卷积操作以处理局部特征
#         self.to_query = nn.Linear(dim, dim * num_heads0)
#         self.to_key = nn.Linear(dim, dim * num_heads0)
#
#         self.w_g = nn.Parameter(torch.randn(dim * num_heads0, 1))
#         self.scale_factor = dim ** -0.5
#         self.Proj = nn.Linear(dim * num_heads0, dim * num_heads0)
#         self.final = nn.Linear(dim * num_heads0, dim)
#
#     def forward(self, x):
#         b, c, h, w = x.shape
#         x = x.unsqueeze(2)
#         # 在第 2 维增加一个维度，将输入 x 变成形状为 (b, c, 1, h, w) 的 5D 张量
#         qkv = self.qkv_dwconv(self.qkv(x))
#         # self.qkv(x): 使用卷积生成查询、键和值。  qkv_dwconv(self.qkv(x)): 对生成的 qkv 进行深度卷积处理。
#         qkv = qkv.squeeze(2)   #去掉第 2 维，恢复成形状 (b, 3*c, h, w)
#         f_conv = qkv.permute(0, 2, 3, 1)    #对 qkv 进行重新排列，用于局部卷积操作
#         f_all = qkv.reshape(f_conv.shape[0], h * w, 3 * self.num_heads, -1).permute(0, 2, 1, 3)
#         # 将 qkv 重塑并传入全连接层 (fc)，处理为特征图
...
#         # 将特征图 f_all 进行重塑和卷积处理，得到局部特征 out_conv
#
#
#         #Swift
#         #print(x.shape)
...
#
#         query_weight = query @ self.w_g  # BxNx1 (BxNxD @ Dx1)
#         #print("query_weight_size",query_weight.shape)
#         ALPHA = query_weight * self.scale_factor  # BxNx1
#         #print("ALPHA_size",ALPHA.shape)
#         ALPHA = torch.nn.functional.normalize(ALPHA, dim=1)  # BxNx1
#         #print("ALPHA_size1",ALPHA.shape)
#         G = torch.sum(ALPHA * query, dim=1)  # BxD
#
#         G = einops.repeat(
#             G, "b d -> b repeat d", repeat=key.shape[1]
#         )  # BxNxD
#         # print("G_size",G.shape)
#         # print("key", key.shape)
#         # print("query_size", query.shape)
#         out = self.Proj(G * key) + query  # BxNxD
#
#         out = self.final(out)  # BxNxD
#
...
#         # edited
#         # # global SA
#         # q, k, v = qkv.chunk(3, dim=1)  # 分离查询、键和值
#
#         # q = rearrange(q, 'b (head c) h w -> b head c (h w)', head=self.num_heads)
#         # k = rearrange(k, 'b (head c) h w -> b head c (h w)', head=self.num_heads)
#         # v = rearrange(v, 'b (head c) h w -> b head c (h w)', head=self.num_heads)
#         # 使用 rearrange 函数对 q, k, v 进行重新排列，以适应多头自注意力机制。
#
...
#         # out = (attn @ v)
#
#         # out = rearrange(out, 'b head c (h w) -> b (head c) h w', head=self.num_heads0, h=h, w=w)
#         # 对自注意力机制的输出进行重新排列
#         # out = out.unsqueeze(2)
