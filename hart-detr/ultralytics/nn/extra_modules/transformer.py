...
from .attention import DAttention, FocusedLinearAttention, HiLo, Combination, EfficientAdditiveAttnetion, CAFM

__all__ = ['TransformerEncoderLayer_LocalWindowAttention', 'AIFI_LPE', 'TransformerEncoderLayer_DAttention', 'TransformerEncoderLayer_HADS', 'TransformerEncoderLayer_EfficientAdditiveAttnetion']
...
class TransformerEncoderLayer_HADS(nn.Module):
    """Defines a single layer of the transformer encoder."""

    def __init__(self, c1, cm=2048, num_heads=8, dropout=0.0, act=nn.GELU(), normalize_before=False):
        """Initialize the TransformerEncoderLayer with specified parameters."""
        super().__init__()
        self.Attn = Combination(c1)
        self.Embed_method = LearnedPositionalEncoding(max_position_embeddings=400,embedding_dim=c1,seq_length=400)
        # Implementation of Feedforward model
        self.fc1 = nn.Conv2d(c1, cm, 1)
        self.fc2 = nn.Conv2d(cm, c1, 1)
    ...
    def forward_post(self, src, src_mask=None, src_key_padding_mask=None, pos=None):
        ...
        # src = src + src1
        B, C, H, W = src.shape
        Embed_code = self.Embed_method(src)
        #print("src的size",src.shape)
        #print("Embed_code的size",Embed_code.shape)
        src = src.flatten(2).permute(0,2,1) + Embed_code
