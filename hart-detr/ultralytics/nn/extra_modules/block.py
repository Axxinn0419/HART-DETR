...
from ..modules.conv import Conv, DWConv, DSConv, RepConv, GhostConv, autopad, LightConv, ConvTranspose, MyConv
...
__all__ = ['Ghost_HGBlock', 'Rep_HGBlock', 'DWRC3', 'C3_DWR', 'C2f_DWR', 'BasicBlock_Ortho', 'BottleNeck_Ortho',
           ...
           'ContextGuideFusionModule', 'C3_DEConv', 'C2f_DEConv', 'BasicBlock_DEConv', 'BottleNeck_DEConv', 'C3_SMPCGLU', 'C2f_SMPCGLU', 'WTConv2d']
...
class BasicBlock_Faster_Block(BasicBlock):
    def __init__(self, ch_in, ch_out, stride, shortcut, act='relu', variant='d'):
        ...
        self.branch2b = ConvolutionalGLU(in_features=ch_out)
...
#
# # _fuse_bn_tensor 方法的作用是将批归一化（BN）的统计信息融合到卷积层的权重和偏置中
#     def _fuse_bn_tensor(self) -> None:
#         kernel = self.conv.weight    # 获取卷积层的权重
#         bias = self.conv.bias if hasattr(self.conv, 'bias') and self.conv.bias is not None else 0
#         # 获取卷积层的偏置，如果存在偏置则使用，否则设为0
#         running_mean = self.bn.running_mean  # 获取批归一化的运行时均值
#         running_var = self.bn.running_var  # 获取批归一化的运行时方差
#         gamma = self.bn.weight  # 获取批归一化的缩放因子（gamma）
#         beta = self.bn.bias   # 获取批归一化的偏移量（beta）
#         eps = self.bn.eps   # 获取批归一化的epsilon值，用于数值稳定性
#         std = (running_var + eps).sqrt()  # 计算标准差，使用批归一化的方差加上epsilon后开平方
#         t = (gamma / std).reshape(-1, 1, 1, 1)  # 根据gamma和std计算一个缩放张量 t，并将其形状reshape为与卷积核相匹配的形状
#         self.conv.weight.data = kernel * t  # 使用缩放张量 t 更新卷积层的权重
#         self.conv.bias = nn.Parameter(beta - (running_mean - bias) * gamma / std, requires_grad=False)
#         # 根据批归一化的统计信息计算并设置卷积层的偏置
#         self.bn = nn.Identity()
#         # 将原来的批归一化层替换为恒等映射 nn.Identity()，从而在推理阶段跳过批归一化的计算
...
class ConvolutionalGLU(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.) -> None:
        ...
        self.dwconv = nn.Sequential(
                ...
                Partial_conv3(hidden_features, n_div=4, forward='split_cat'),
            act_layer()
        )
...
class Star_Block(nn.Module):
    def __init__(self, dim, mlp_ratio=3, drop_path=0.):
        super().__init__()
        self.Mypconv = Conv(dim, dim, 7, g=dim, act=False)   # 它的目的是通过深度可分离卷积提取局部特征
        ...
        # 它的作用是处理特征图的空间信息，并通过深度可分离卷积进一步提取特征
        self.act = nn.ReLU6()
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
...
#     def __init__(self, dim, mlp_ratio=3, drop_path=0.):
#         super().__init__()
#         self.dwconv = Conv(dim, dim, 7, g=dim, act=False)   # 它的目的是通过深度可分离卷积提取局部特征
#         self.f1 = nn.Conv2d(dim, mlp_ratio * dim, 1)
#         self.f2 = nn.Conv2d(dim, mlp_ratio * dim, 1)
#         self.g = Conv(mlp_ratio * dim, dim, 1, act=False)
#         self.dwconv2 = nn.Conv2d(dim, dim, 7, 1, (7 - 1) // 2, groups=dim)
...
class SDFM(nn.Module):
    ...
    def __init__(self, channels=64, r=4):
        ...
        self.EfficientLocalizationAttention = EfficientLocalizationAttention(channel = 2 , kernel_size=7)
    ...
    def forward(self, data):
        ...
        agg_input = self.channel_agg(input)
        # local_w = self.local_att(agg_input)  #上

        local_w =  self.EfficientLocalizationAttention(agg_input)  # 上

        global_w = self.global_att(agg_input)  #下
        w = self.sigmoid(local_w * global_w)  ## 计算特征x1的权重

        xo = (w * x1 + x2) + ((1 - w) * x2 + x1)

        return xo

######################################### BasicBlock_Faster_Block_CGLU#####################################################k
